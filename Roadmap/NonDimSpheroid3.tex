...
\end{eqnarray}
Equations \ref{equvol3} and \ref{equvol5} are now the basis of a nondimensionalization in which tissue volume is factored out of the rescaled equations.
...
%	%D_{sph}^* = \sqrt[\uproot{5}3]{\alpha_s} D_t^*
%	%\frac{\pi}{6} {D_{sph}^*}^3 = \frac{\pi}{6} \alpha_s \beta {D_t^*}^3
%\end{eqnarray} 
%from which
%\begin{eqnarray}\label{equivsphere4}
%	%\frac{\pi}{6} {D_{sph}^*}^3 = \frac{\pi}{6} \alpha_s {D_t^*}^3, \non \\
...
\end{itemize}
%This makes $T^*$ as defined in Equation \ref{charT} suitable as a characteristic value to factor out direct effects of size, fluid medium density and gravity.
...
\begin{itemize}
	...
	\item Scaling for force (for $q \in x, y, z$):
		\begin{eqnarray*}
			...
			%f_q \equiv \frac{\tau^*_{rot} D_{sph}^*}{\mu^*} f_q^*, q \in x, y, z
			%f_q^* = \frac{\mu^*}{\tau^*_{rot} D_{sph}^*} f_q, \\
    \end{eqnarray*} 
...
Densities are often estimated from equilibrium positions in water columns with density gradients (to measure average density) or from known properties of specific materials (seawater, lipids, calcium carbonate, \etc). 
...
\begin{enumerate}
	...
	\begin{eqnarray}\label{d2ndpars1}
		...
		\eta_{i} = \frac{L^*_{i_2}}{L^*_{i_0}}, ~ \xi = \frac{h_i^*}{L_{s_0}^*}
	\end{eqnarray}
	...
	\end{eqnarray}
	\item Calculate the equivalent sphere diameter, the characteristic length and the rotation timescale:
...
%\textbf{May be useful to define an inclusion volume parameter, $V_i/V_t$.}
	...
	\begin{eqnarray}\label{d2ndpars5}
		...
		\frac{\p w}{\p x} = \frac{1}{\l ~ \tau^*_{rot}} \frac{\p w^*}{\p x^*} \non \\
		\frac{\p u}{\p z} = \frac{1}{\l ~ \tau^*_{rot}} \frac{\p u^*}{\p z^*} \non \\
